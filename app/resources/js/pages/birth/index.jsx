...
export function BirthListPage() {
    ...
    const handleDelete = async (recordId) => {
        ...
        try {
            ...
            await loadBirths();
            setSuccessMessage('Birth record deleted successfully.');
        } catch (error) {
            setErrorMessage(error instanceof Error ? error.message : 'Unexpected error.');
        } finally {
            setDeletingId(null);
    ...
    const handleExportCsv = async () => {
        ...
        try {
            ...
            const exportUrl = `/api/births/export-csv${params.toString() ? `?${params.toString()}` : ''}`;
            ...
            const filename = filenameMatch?.[1] || `births-${new Date().toISOString().slice(0, 10)}.csv`;
            ...
            setSuccessMessage('Birth records exported successfully.');
        } catch (error) {
    ...
    const handleImportCsv = async (event) => {
        ...
        try {
            ...
            const response = await api.post('/api/births/import-csv', formData, {
                headers: {
